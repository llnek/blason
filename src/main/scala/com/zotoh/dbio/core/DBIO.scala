...
import com.zotoh.frwk.db.JDBCUtils.setStatement
import com.zotoh.frwk.db.JDBCUtils.toSqlType
import com.zotoh.frwk.util.StrUtils.addAndDelim
import com.zotoh.dbio.core.MetaCache._
...
object DBIO {
  private val _log= LoggerFactory.getLogger(classOf[DBIO])
}
...
class DBIO {
  ...
  def fetchSQL(sql:String , pms:Any*) = {
    _jdbc.fetchRows( new SELECTStmt(sql, pms:_*) )
  }

  def execUpdateSQL(sql:String , pms:Any* ) = {
    val conn= if (isTransacting) txObj else _pool.nextFree()
    var ps:PreparedStatement = null
    try {
      ps= conn.prepareStatement( jiggleSQL(sql, -1) )
      pms.foldLeft(1) { (pos,p) =>
        setStatement(ps, pos, p)
        p+1
      }
      ps.executeUpdate()
      if (!isTransacting) { conn.commit }
    } catch {
      case e:Throwable =>
      if (!isTransacting) { conn.rollback }
      throw e
    } finally {
      DBU.close(ps)
      if ( ! isTransacting) { DBU.closeQuietly(conn) }
    }
  }
  ...
  def countRows(target:Class[_]) = {
    getTableMeta(target) match {
      case null => 0
      case tm => _jdbc.countRows( tm.getName )
    }
  }

  def List<Object> callProcAsFunction(String name, List<Object> params, List< Class<?> > outs)
