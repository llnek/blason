...
(defn hasTZPart?
  ...
  [dateStr]
  (let [ tkns (.split (SU/nsb dateStr) (if (SU/contains? dateStr \:) CS/TS_REGEX CS/DT_REGEX )) ]
    (some (fn [s]
            (or (SU/hasWithin (SU/nsb s) ["+" "-"])
...
(defn parseDateISO8601
  ...
  (if (StringUtils/isEmpty tstr)
    nil
    (let [ fmt (if (SU/contains? tstr \:)
                  (let [ s (if (SU/contains? tstr \.) CS/DT_FMT_MICRO CS/DT_FMT ) ]
